import { int } from '../../data/primitives';
import { Relation } from '../../data/query/Relations';
import { DBCIntCell, DBCKeyCell } from '../../data/dbc/DBCCell';
import { DBCFile } from '../../data/dbc/DBCFile';
import { DBCRow } from '../../data/dbc/DBCRow';
/**
 * Main row definition
 * - Add column comments to the commented getters below
 * - Add file comments to DBCFiles.ts
 */
export declare class TaxiPathRow extends DBCRow<TaxiPathCreator, TaxiPathQuery> {
    /**
     * Primary Key
     *
     * No comment (yet!)
     */
    get ID(): DBCKeyCell<this>;
    /**
     * No comment (yet!)
     */
    get FromTaxiNode(): DBCIntCell<this>;
    /**
     * No comment (yet!)
     */
    get ToTaxiNode(): DBCIntCell<this>;
    /**
     * No comment (yet!)
     */
    get Cost(): DBCIntCell<this>;
    /**
     * Creates a clone of this row with new primary keys.
     *
     * Cloned rows are automatically added at the end of the DBC file.
     */
    clone(ID: int, c?: TaxiPathCreator): this;
}
/**
 * Used for object creation (Don't comment these)
 */
export type TaxiPathCreator = {
    FromTaxiNode?: int;
    ToTaxiNode?: int;
    Cost?: int;
};
/**
 * Used for queries (Don't comment these)
 */
export type TaxiPathQuery = {
    ID?: Relation<int>;
    FromTaxiNode?: Relation<int>;
    ToTaxiNode?: Relation<int>;
    Cost?: Relation<int>;
};
/**
 * Table definition (specifies arguments to 'add' function)
 * - Add file comments to DBCFiles.ts
 */
export declare class TaxiPathDBCFile extends DBCFile<TaxiPathCreator, TaxiPathQuery, TaxiPathRow> {
    constructor();
    /** Loads a new TaxiPath.dbc from a file. */
    static read(path: string): TaxiPathDBCFile;
    add(ID: int, c?: TaxiPathCreator): TaxiPathRow;
    findById(id: number): TaxiPathRow;
}
