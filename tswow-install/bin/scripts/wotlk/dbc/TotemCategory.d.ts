import { int, loc_constructor } from '../../data/primitives';
import { Relation } from '../../data/query/Relations';
import { DBCIntCell, DBCKeyCell, DBCLocCell } from '../../data/dbc/DBCCell';
import { DBCFile } from '../../data/dbc/DBCFile';
import { DBCRow } from '../../data/dbc/DBCRow';
/**
 * Main row definition
 * - Add column comments to the commented getters below
 * - Add file comments to DBCFiles.ts
 */
export declare class TotemCategoryRow extends DBCRow<TotemCategoryCreator, TotemCategoryQuery> {
    /**
     * Primary Key
     *
     * No comment (yet!)
     */
    get ID(): DBCKeyCell<this>;
    /**
     * No comment (yet!)
     */
    get Name(): DBCLocCell<this>;
    /**
     * No comment (yet!)
     */
    get TotemCategoryType(): DBCIntCell<this>;
    /**
     * No comment (yet!)
     */
    get TotemCategoryMask(): DBCIntCell<this>;
    /**
     * Creates a clone of this row with new primary keys.
     *
     * Cloned rows are automatically added at the end of the DBC file.
     */
    clone(ID: int, c?: TotemCategoryCreator): this;
}
/**
 * Used for object creation (Don't comment these)
 */
export type TotemCategoryCreator = {
    Name?: loc_constructor;
    TotemCategoryType?: int;
    TotemCategoryMask?: int;
};
/**
 * Used for queries (Don't comment these)
 */
export type TotemCategoryQuery = {
    ID?: Relation<int>;
    Name?: Relation<string>;
    TotemCategoryType?: Relation<int>;
    TotemCategoryMask?: Relation<int>;
};
/**
 * Table definition (specifies arguments to 'add' function)
 * - Add file comments to DBCFiles.ts
 */
export declare class TotemCategoryDBCFile extends DBCFile<TotemCategoryCreator, TotemCategoryQuery, TotemCategoryRow> {
    constructor();
    /** Loads a new TotemCategory.dbc from a file. */
    static read(path: string): TotemCategoryDBCFile;
    add(ID: int, c?: TotemCategoryCreator): TotemCategoryRow;
    findById(id: number): TotemCategoryRow;
}
