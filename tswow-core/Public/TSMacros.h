...
#define EVENT_HANDLE(category,name) void name(category##name##__Type cb) { Add(this->events->category##name.Add(cb)); }
#define EVENT_HANDLE_FN(category,name,fn) void name(category##name##__Type cb) { Add(this->events->category##name.Add(cb)); fn(cb,std::numeric_limits<uint32_t>::max()); }
#define MAP_EVENT_HANDLE(category,name) void name(uint32 id, category##name##__Type cb) { Add(this->eventMap->Get(id)->category##name.Add(cb));}
#define MAP_EVENT_HANDLE_FN(category,name,fn) void name(uint32 id, category##name##__Type cb) { Add(this->eventMap->Get(id)->category##name.Add(cb)); fn(cb,id); }
