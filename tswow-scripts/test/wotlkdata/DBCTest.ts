...
describe('DBC', function() {
    ...
    describe('Add', function() {
        it('works with multiple id field files', function() {
            ...
            ic.read(TEMP_DBC);
            assert.strictEqual(ic.rowCount, 18);
            assert.strictEqual(ic.find({ClassID: 1007688}).Flags.get(), 123123);
        });
    ...
    it('writes loc_constructors', function() {
        ...
        ipf.write(TEMP_DBC);
        ipf.read(TEMP_DBC);
        assert.strictEqual(ipf.find({ID: 1007688}).Name.enGB.get(), 'testname');
    });
    ...
    it('writes multi-id files', function() {
        ...
        ic.read();
        assert.strictEqual(ic.rowCount, 17);
        ic.find({}).clone(1007688, 1007688, {Flags: 123123});
        assert.strictEqual(ic.rowCount, 18);
        ic.write(TEMP_DBC);
        ic.read();
        assert.strictEqual(ic.rowCount, 17);
    ...
    it('writes single-id files', function() {
        ...
        ipcg.read();
        assert.strictEqual(ipcg.rowCount, 1);
        const grp = ipcg.find({}).clone(1007688);
        grp.ItemID.setIndex(0, 123123);
        assert.strictEqual(ipcg.rowCount, 2);
        ipcg.write(TEMP_DBC);
        ipcg.read();
    ...
    describe('SpecificFiles', function() {
        describe('Spell', function() {
            ...
            it('Has "Blizzard" at ID=10', function() {
                const blizzard = DBC.Spell.findById(10);
        ...
        describe('ChrClasses', function() {
            it('Has "Mage" at ID=8', function() {
                const t = DBC.ChrClasses.find({ID: 8});
            ...
            it('Has required_expansion=2 at Filename="DEATHKNIGHT"', function() {
                const t = DBC.ChrClasses.find({Filename: 'DEATHKNIGHT'});
        ...
        describe('gtOCTClassCombatRatingScalar (gt file with IDs)', function() {
            ...
            it('Has Data=1.29999995231628 at ID=338', function() {
                assert.strictEqual(Math.abs(DBC.GtOCTClassCombatRatingScalar
                    .find({ID: 338}).Data.get() - 1.29999995231628) < 0.0000000001, true);
    ...
    describe('DBCFile#add', function() {
        ...
        it('keeps added rows', function () {
            DBC.Languages.read();
            assert.strictEqual(DBC.Languages.find({ID: 39}), undefined);
            DBC.Languages.add(39);
            const row = DBC.Languages.find({ID: 39});
        ...
        it('keeps added rows on disk', function () {
            ...
            assert.notStrictEqual(DBC.Languages.find({ID: 39}), undefined);
        });
    });
    ...
    describe('Modifying', function() {
        it('modifies values in memory',  function() {
            DBC.Languages.read();
            const taurahe = DBC.Languages.find({ID: 3});
        ...
        it('keeps modifications on disk', function () {
            ...
            assert.strictEqual(DBC.Languages
                .find({ID: 3}).Name.enGB.get(), 'Tauraho');
        });
        ...
        it('combines modifications and additions', function() {
            ...
            assert.strictEqual(DBC.Languages.find({ID: 3}).Name.enGB.get(), 'Taur');
            assert.strictEqual(DBC.Languages.find({ID: 50}).Name.enGB.get(), 'TestLang');
    ...
    describe('BinSearch', function() {
        it('finds valid ids', function() {
            assert.strictEqual(DBC.Spell.findById(25).ID.get(), 25);
            assert.strictEqual(DBC.Spell.findById(1).ID.get(), 1);
            assert.strictEqual(DBC.Spell.findById(80864).ID.get(), 80864);
        });
        ...
        it('finds modded ids', function() {
            ...
            assert.strictEqual(DBC.SkillLine.findById(3500).ID.get(), 3500);
            assert.strictEqual(DBC.SkillLine.findById(788).ID.get(), 788);
            assert.strictEqual(DBC.SkillLine.findById(6).ID.get(), 6);
            assert.strictEqual(DBC.SkillLine.findById(3999).ID.get(), 3999);
            assert.strictEqual(DBC.SkillLine.findById(3000).ID.get(), 3000);
        });
    });

    describe('DBCBufferArray', function() {
        it('Reads', function() {
            const r1 = DBC.Spell.findById(3591);
        ...
        it('Reads non-numeric values', function() {
            DBC.Vehicle.read();
            const r = DBC.Vehicle.findById(24);
