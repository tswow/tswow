...
class Connection {
    ...
    /**
     * Rebuilds the database for this connection using the sql files generated by trinitycore at compile-time.
     *
     * @returns Promise that resolves once the database has been fully rebuilt.
     */
    async rebuild() {
        await this.status;
        await promiseConnect(this);
        await promiseQuery(this.con, `DROP DATABASE IF EXISTS \`${this.name()}\`;`);
        this.status = undefined;
        await this.connect();
        const sqlpath = isWindows() ? `"bin/mysql/bin/mysql.exe"` : `sudo mysql`;
        await wsys.execAsync(`${sqlpath} -u root ${
            this.name()} < bin/sql/trinitycore_${
            this.type === 'world_source' ? 'world' : this.type}.sql`);
        term.success(`Rebuilt ${this.type} database (${this.name()})`);
    }
...
export namespace mysql {
    ...
    /**
     * Rebuilds all TrinityCore databases.
     */
    export async function rebuildAll() {
        return await Promise.all(mysql.all.map((x) => x.rebuild()));
    }
    ...
    export async function initialize() {
        ...
        mysqlC.addCommand('rebuild', 'world|auth|characters|all', 'Rebuilds the entire mysql database', async(args: string[]) => {
            if (args[0] === 'all') {
                await rebuildAll();
            } else {
                await getDatabase(args[0]).rebuild();
            }
        });
